...
-
### Part 5.1 - Create `com.github.curriculeon.Instructor` Class
* Create an `com.github.curriculeon.Instructor` class such that:
	* `com.github.curriculeon.Instructor` is a subclass of `Person`
	* `com.github.curriculeon.Instructor` implements the `Teacher` interface
	* `com.github.curriculeon.Instructor` should have a concrete implementation of the `teach` method which invokes the `learn` method on the specified `Learner` object.
	* `com.github.curriculeon.Instructor` should have a concrete implementation of the `lecture` method which invokes the `learn` method on each of the elements in the specified array of `Learner` objects.
		* `numberOfHours` should be evenly split amongst the learners.
			* `double numberOfHoursPerLearner = numberOfHours / learners.length;`

-
### Part 5.0 - Test `com.github.curriculeon.Instructor`
* Create a `TestInstructor` class.
	* Create a `testImplementation` method that asserts that an `com.github.curriculeon.Instructor` is an `instanceof` a `Teacher`.
	* Create a `testInheritance` method that asserts that a `com.github.curriculeon.Instructor` is an `instanceof` a `Person`.
	* Create a `testTeach` method that ensures when an `com.github.curriculeon.Instructor` invokes the `teach` method, a respective student's `totalStudyTime` instance variable is incremented by the specified `numberOfHours`.
	* Create a `testLecture` method that ensures when an `com.github.curriculeon.Instructor` invokes the `lecture` method, a respective array of students' `totalStudyTime` instance variables is incremented by `numberOfHours/students.length`.
...
* Create a `Classroom` singleton.
	...
	* The class should define a method `hostLecture` which makes use of a `long id, double numberOfHours` parameter to identify a respective `com.github.curriculeon.Instructor` to host a `lecture` to the composite `personList` field in the `cohort` reference.
...
* Modify the `Instructors` class signature to ensure that it is a subclass of `People` of parameterized type `com.github.curriculeon.Instructor`.
...
* Create an enum named `Educator`.
	...
	* Upon construction each enumeration of the enum should instantiate a respective `com.github.curriculeon.Instructor` and assign it to a final `instructor` field upon construction. The `instructor` should be added to the `Instructors` singleton.
